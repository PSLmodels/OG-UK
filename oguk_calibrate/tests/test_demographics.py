import numpy as np
from oguk_calibrate import demographics
...
def test_pop_smooth():
    ...
    assert np.any(
        np.absolute(pop_dict["omega"][:-1, :] - pop_dict["omega"][1:, :]) < 0.0001
    )
    assert np.any(np.absolute(pop_dict["g_n"][:-1] - pop_dict["g_n"][1:]) < 0.0001)
...
def test_imm_smooth():
    ...
    assert np.any(
        np.absolute(pop_dict["imm_rates"][:-1, :] - pop_dict["imm_rates"][1:, :])
        < 0.0001
    )
